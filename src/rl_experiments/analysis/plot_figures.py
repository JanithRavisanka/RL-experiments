...
from rl_experiments.analysis.plot_style import (
    ALGO_COLORS,
    DARK_BG,
    GRID_COLOR,
    PANEL_BG,
...
def plot_cartpole_comparison(log_dir: str = "logs", out_dir: str = "analysis/figures"):
    ...
    ddqn_dfs    = load_custom_logs("**/double_dqn_cartpole_v1_*.csv", log_dir)
    per_dqn_dfs = load_custom_logs("**/per_dqn_cartpole_v1_*.csv", log_dir)
    rainbow_dfs = load_custom_logs("**/rainbow_cartpole_v1_*.csv", log_dir)
    ...
    plot_mean_std_band(ax, ddqn_dfs, "timestep", "reward", "DoubleDQN", ALGO_COLORS["DoubleDQN"])
    plot_mean_std_band(ax, per_dqn_dfs, "timestep", "reward", "PER-DQN", ALGO_COLORS["PER-DQN"])
    plot_mean_std_band(ax, rainbow_dfs, "timestep", "reward", "Rainbow", ALGO_COLORS["Rainbow"])

    _style_ax(ax,
        title  = "CartPole-v1 — DQN Family + PPO  (mean ± std)",
...
def plot_paradigm_comparison(log_dir: str = "logs", out_dir: str = "analysis/figures"):
    ...
    logs = {
        ...
        "DoubleDQN": load_custom_logs("**/double_dqn_cartpole_v1_*.csv", log_dir),
        "PER-DQN":   load_custom_logs("**/per_dqn_cartpole_v1_*.csv", log_dir),
        "Rainbow":   load_custom_logs("**/rainbow_cartpole_v1_*.csv", log_dir),
        "Dreamer":   load_custom_logs("**/dreamer_cartpole_v1*.csv", log_dir),
        "MuZero":    load_custom_logs("**/muzero_cartpole_v1*.csv", log_dir),
        "PlaNet":    load_custom_logs("**/planet_cartpole_v1*.csv", log_dir),
        "WorldModels": load_custom_logs("**/world_models_cartpole_v1*.csv", log_dir),
        "I2A":       load_custom_logs("**/i2a_cartpole_v1*.csv", log_dir),
    }
    ...
    for algo, dfs in logs.items():
        ...
        if algo in ("PPO", "DQN"):
            ...
            x_col = "episode" if "episode" in dfs[0].columns else "timestep"
            y_col = "reward"
...
def plot_sample_efficiency(log_dir: str = "logs", out_dir: str = "analysis/figures"):
    ...
    data = {
        ...
        "DoubleDQN": ("**/double_dqn_cartpole_v1_*.csv", "timestep", "reward"),
        "PER-DQN":   ("**/per_dqn_cartpole_v1_*.csv", "timestep", "reward"),
        "Rainbow":   ("**/rainbow_cartpole_v1_*.csv", "timestep", "reward"),
        "Dreamer":   ("**/dreamer_cartpole_v1*.csv", "episode", "reward"),
        "MuZero":    ("**/muzero_cartpole_v1*.csv", "episode", "reward"),
        "PlaNet":    ("**/planet_cartpole_v1*.csv", "episode", "reward"),
        "WorldModels": ("**/world_models_cartpole_v1*.csv", "episode", "reward"),
        "I2A":       ("**/i2a_cartpole_v1*.csv", "step", "reward"),
        "PETS":      ("**/pets_pendulum_v1*.csv", "step", "reward"),
        "MBPO":      ("**/mbpo_pendulum_v1*.csv", "step", "reward"),
        "TD-MPC":    ("**/tdmpc_pendulum_v1*.csv", "step", "reward"),
        "TD-MPC2":   ("**/tdmpc2_pendulum_v1*.csv", "step", "reward"),
        "MVE":       ("**/mve_pendulum_v1*.csv", "step", "reward"),
        "STEVE":     ("**/steve_pendulum_v1*.csv", "step", "reward"),
    }
